...
from monai.losses import TverskyLoss, DiceLoss, GeneralizedDiceFocalLoss, FocalLoss
...
def main():
    ...
    parser.add_argument('--config', type=str, default="/data/hyungseok/Swin-UNETR/api/exp2.yaml", help="Path to the YAML config file")
   ...
   # Dataset 구성
    ...
    elif config['train_params']['loss_type'] == "DiceFocalLoss":
        loss_function = DiceFocalLoss(to_onehot_y=True,softmax=True,gamma=2.0,lambda_dice=1.0,lambda_focal=1.0)
    elif config['train_params']['loss_type'] == "TverskyLoss":
        loss_function = TverskyLoss(to_onehot_y=True,softmax=True,alpha=0.3,beta=0.7) #alpha FP weight beta FN weight
    elif config['train_params']['loss_type'] == "GeneralizedDiceFocalLoss":
        loss_function = GeneralizedDiceFocalLoss(to_onehot_y=True,softmax=True,gamma=1.5) #class별 weighting 자동 적용 (volume inverse)
