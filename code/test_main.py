...
from utils.my_utils import worker_init_fn, TqdmLoggingHandler, setup_logging
...
### 5. Evaluation
# def evaluate_metrics(pred, gt, metric_pool: Dict[str, object], organ: str) -> Dict[str, float]:
#     """
#     Dice, HD95, ASD, component recall 등 config에 따라 선택적 계산
#     이 함수에서는 각 이미지에 대해서만 진행할 예정
#     전체 지표등은 main_loop에서 관리해서 디버깅시 메인 루프만 봐도 각 값이 보여지도록
#     """
#     temp_dice_metric = DiceMetric(include_background=True, reduction="mean", get_not_nans=False)
#     temp_dice_metric(y_pred=pred, y=gt)
...
# def render_and_save(image, label, pred, save_dir: str, case_name: str, organ: str, config: Dict):
#     """
#     시각화 여부에 따라 render_overlay 수행
#     save_3d_results 호출 포함
#     """
#     pass
...
def main():
    ...
    import sys
    sys.path.append('/data/hyungseok/Swin-UNETR/')
    from utils.my_utils import save_config_as_json, save_current_code
    ...
    for organ in organ_list:
        logging.info(f"[Summary] {organ.upper()} — " + ", ".join([
