...
def main():
    ...
    parser.add_argument('--config', type=str, default="/data/hyungseok/Swin-UNETR/api/lung.yaml", help="Path to the YAML config file")
    ...
    lung_train_transforms = Compose(
        [
            ...
            RandScaleIntensityd(
                keys=["image"],
                factors=config['transforms']['lung']['rand_scale_intensity_factor'], 
                prob=config['transforms']['lung']['rand_scale_intensity_prob']),
            RandShiftIntensityd(
                keys=["image"],
   ...
   # Dataset 구성
    ...
    lung_val_ds = PersistentDataset(data=lung_val_files, transform=lung_val_transforms,cache_dir="/data/hyungseok/Swin-UNETR/data_cache/Liver_Lung/Lung/val")                                                       
    ...
    def validation(epoch_iterator_val_lung):
        model.eval()
        dice_metric.reset()  # Reset metric at the start of validation
        with torch.no_grad():
            for step, batch in enumerate(epoch_iterator_val_lung):
                ...
                epoch_iterator_val_lung.set_description(
                    f"Validate lung ({step + 1} / {len(epoch_iterator_val_lung)} Steps)"
                )
    ...
    while epoch < max_epoch:
        epoch_iterator_train = tqdm(
                lung_train_loader, desc="Training (X / X Steps) (dice=X.X)", dynamic_ncols=True
            )
        ...
        if (epoch % eval_epoch == 0 and epoch != 0) or epoch == max_epoch:
            ...
            dice_val = validation(lung_epoch_iterator_val)
