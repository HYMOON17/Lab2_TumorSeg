...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        logits = self.out(out)
        # Attention map 수집
        attn_maps = self.swinViT.get_collected_maps()
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn)
        attn_map = attn  # Attention map 저장
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        # return x
        return {"x": x, "attn_map": attn_map}  # 딕셔너리로 반환
    ...
    def forward(self, x, mask_matrix, collector=None):
        shortcut = x
        if self.use_checkpoint:
            outputs = checkpoint.checkpoint(self.forward_part1, x, mask_matrix, use_reentrant=False)
        else:
            outputs = self.forward_part1(x, mask_matrix)
        # 딕셔너리에서 반환값 분리
        x = outputs["x"]
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, collector=None):
        x_shape = x.size()
        if len(x_shape) == 5:
            b, c, d, h, w = x_shape
            window_size, shift_size = get_window_size((d, h, w), self.window_size, self.shift_size)
            ...
            for blk in self.blocks:
                x = blk(x, attn_mask, collector=collector)
            x = x.view(b, d, h, w, -1)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.use_v2 = use_v2
        self.collector = AttentionMapCollector()  # Collector를 SwinTransformer에 통합
    ...
    def forward(self, x, normalize=True):
        self.reset_collector()  # Collector 초기화
        x0 = self.patch_embed(x)
        x0 = self.pos_drop(x0)
        x0_out = self.proj_out(x0, normalize)
        if self.use_v2:
            x0 = self.layers1c[0](x0.contiguous())
        x1 = self.layers1[0](x0.contiguous(), collector=self.collector)
        x1_out = self.proj_out(x1, normalize)
        if self.use_v2:
            x1 = self.layers2c[0](x1.contiguous())
        x2 = self.layers2[0](x1.contiguous(), collector=self.collector)
        x2_out = self.proj_out(x2, normalize)
        if self.use_v2:
            x2 = self.layers3c[0](x2.contiguous())
        x3 = self.layers3[0](x2.contiguous(), collector=self.collector)
        x3_out = self.proj_out(x3, normalize)
        if self.use_v2:
            x3 = self.layers4c[0](x3.contiguous())
        x4 = self.layers4[0](x3.contiguous(), collector=self.collector)
